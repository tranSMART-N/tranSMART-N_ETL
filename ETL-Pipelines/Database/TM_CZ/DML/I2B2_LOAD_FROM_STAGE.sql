  CREATE OR REPLACE PROCEDURE TM_CZ.I2B2_LOAD_FROM_STAGE
(
  VARCHAR(100)
  ,varchar(500)
  ,bigint
...
BEGIN
	...
	loop	
		...
		if r_stage_table.study_specific = 'N' then	
			...
			stepCt := stepCt + 1;		
			call tm_cz.czx_write_audit(jobId,databaseName,procedureName,'Truncated '|| source_table,0,stepCt,'Done');
			--	insert from staged source into target
			tText := 'insert into ' || source_table || ' select st.* from ' || release_table || ' st ';
			execute immediate tText;
			rowCt := ROW_COUNT;
			stepCt := stepCt + 1;
			call tm_cz.czx_write_audit(jobId,databaseName,procedureName,'Inserted all data into ' || source_table,rowCt,stepCt,'Done');		
		else
			tText := 'delete from ' || source_table || ' st ' || replace(r_stage_table.where_clause,'TrialId','''' || TrialId || '''');
			execute immediate tText;
			rowCt := ROW_COUNT;
			stepCt := stepCt + 1;
			call tm_cz.czx_write_audit(jobId,databaseName,procedureName,'Deleted study from ' || source_table,ROW_COUNT,stepCt,'Done');
			...
			rowCt := ROW_COUNT;
			stepCt := stepCt + 1;
			call tm_cz.czx_write_audit(jobId,databaseName,procedureName,'Inserted study into ' || source_table,rowCt,stepCt,'Done');
		end if;		
	...
	if pCount = 0 then
		stepCt := stepCt + 1;
		call tm_cz.czx_write_audit(jobId,databaseName,procedureName,'No staged data for study',0,stepCt,'Done');
		return 16;
	end if;	
	...
	if instr(dataType,'CLINICAL') > 0 then
		...
		if topNode is null then
			stepCt := stepCt + 1;
			call tm_cz.czx_write_audit(jobId,databaseName,procedureName,'Unable to get topNode for study',0,stepCt,'Done');
			return 16;
		end if;
		...
		if pCount > 2 then
			...
			call i2b2_fill_in_tree(null, tPath, jobId);
		end if;
