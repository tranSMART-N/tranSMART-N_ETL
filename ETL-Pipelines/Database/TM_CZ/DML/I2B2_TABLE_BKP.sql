...
******************************************************************/
	...
	dbms_output.put_line('tExists: ' || to_char(tExists));
	if tExists > 0 then
		execute immediate('drop table i2b2metadata.i2b2_bkp');
		dbms_output.put_line('drop i2b2_bkp');
	end if;
	...
	from all_tables
	where owner = 'I2B2DEMODATA'
      and table_name = 'VISIT_DIMENSION_BKP';
	
	if tExists > 0 then
		execute immediate('drop table i2b2demodata.visit_dimension_bkp');
	end if;
	  
 	select count(*) into tExists
	from all_tables
	where owner = 'I2B2DEMODATA'
      and table_name = 'MODIFIER_DIMENSION_BKP';
	
	if tExists > 0 then
		execute immediate('drop table i2b2demodata.modifier_dimension_bkp');
	end if;
	
	--Backup tables
	dbms_output.put_line('start backup');
	EXECUTE IMMEDIATE 'CREATE TABLE I2B2METADATA.I2B2_BKP AS SELECT * FROM I2B2METADATA.I2B2';
	dbms_output.put_line('i2b2_bkp created');
	EXECUTE IMMEDIATE 'CREATE TABLE I2B2DEMODATA.CONCEPT_COUNTS_BKP AS SELECT * FROM I2B2DEMODATA.CONCEPT_COUNTS';
	dbms_output.put_line('concept_counts_bkp created');
	EXECUTE IMMEDIATE 'CREATE TABLE I2B2DEMODATA.CONCEPT_DIMENSION_BKP AS SELECT * FROM I2B2DEMODATA.CONCEPT_DIMENSION';
	dbms_output.put_line('concept_dimension_bkp created');
	EXECUTE IMMEDIATE 'CREATE TABLE I2B2DEMODATA.OBSERVATION_FACT_BKP AS SELECT * FROM I2B2DEMODATA.OBSERVATION_FACT';
	dbms_output.put_line('observation_fact_bkp created');
	EXECUTE IMMEDIATE 'CREATE TABLE I2B2DEMODATA.PATIENT_DIMENSION_BKP AS SELECT * FROM I2B2DEMODATA.PATIENT_DIMENSION';
	dbms_output.put_line('patient_dimension_bkp created');
	EXECUTE IMMEDIATE 'CREATE TABLE I2B2DEMODATA.VISIT_DIMENSION_BKP AS SELECT * FROM I2B2DEMODATA.VISIT_DIMENSION';
	dbms_output.put_line('patient_dimension_bkp created');
	EXECUTE IMMEDIATE 'CREATE TABLE I2B2DEMODATA.MODIFIER_DIMENSION_BKP AS SELECT * FROM I2B2DEMODATA.MODIFIER_DIMENSION';
	dbms_output.put_line('patient_dimension_bkp created');
END;
