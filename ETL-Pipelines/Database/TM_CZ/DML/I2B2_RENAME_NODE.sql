...
BEGIN
  ...
  stepCt := 0;
	...
	stepCt := stepCt + 1;
	czx_write_audit(jobId,databaseName,procedureName,'Start i2b2_rename_node',0,stepCt,'Done'); 
  ...
  then
	...
	stepCt := stepCt + 1;
	czx_write_audit(jobId,databaseName,procedureName,'Update concept_counts with new path',SQL%ROWCOUNT,stepCt,'Done'); 
      ...
      where t.path in
		   (select cd.concept_path from concept_dimension cd
		    where cd.sourcesystem_cd = trial_id
              and cd.concept_path like '%\' || old_node || '\%');
	stepCt := stepCt + 1;
	czx_write_audit(jobId,databaseName,procedureName,'Update i2b2_tags with new path',SQL%ROWCOUNT,stepCt,'Done'); 
